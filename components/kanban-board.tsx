...
import { getCurrentUser, getVendedorNameForUser } from "@/lib/auth"
...
const KANBAN_INITIAL_STAGE_LIMIT = 25
const KANBAN_STAGE_INCREMENT = 25
...
export function KanbanBoard({ empresaId }: KanbanBoardProps) {
  ...
  useEffect(() => {
    setStageLimits({})
  }, [searchTerm, filterOrigem, filterEstagio, visibleColumns.join("|")])
  ...
  const loadLeads = async () => {
    ...
    if (user) {
      const cargo = (user.cargo || "").toLowerCase()
      const columnsToLoad = cargo === "vendedor" ? COLUNAS_VENDEDOR : COLUNAS_ADMIN
      const sellerFilter = cargo === "vendedor" ? await getVendedorNameForUser(user) : undefined
      const [stagePages, vendedoresData] = await Promise.all([
        getLeadsStagePages(user.id_empresa, columnsToLoad, KANBAN_INITIAL_STAGE_LIMIT, { vendedor: sellerFilter }),
        getVendedores(user.id_empresa),
      ])

      const data = stagePages.flatMap((stagePage) => stagePage.leads)
      const counts = stagePages.reduce<Record<string, number>>((acc, stagePage) => {
        acc[stagePage.stage] = stagePage.count
        return acc
      }, {})

      setVisibleColumns(columnsToLoad)
      setLeads(data)
      setStageCounts(counts)
      setTotalLeadsCount(Object.values(counts).reduce((total, count) => total + count, 0))
  ...
  const loadFullLeadsForList = async () => {
    ...
    const cargo = (user.cargo || "").toLowerCase()
    const sellerName = cargo === "vendedor" ? await getVendedorNameForUser(user) : user.nome_usuario
    const filteredData =
      cargo === "vendedor"
        ? data.filter((lead) => normalizeSellerName(lead.vendedor) === normalizeSellerName(sellerName))
        : data

    setLeads(filteredData)
    setTotalLeadsCount(filteredData.length)
    ...
    if (normalizedSearch) {
      filtered = filtered.filter(
        (lead) =>
          ...
          lead.telefone?.includes(searchTerm.trim()) ||
          lead.vendedor?.toLowerCase().includes(normalizedSearch),
      )
    }
  ...
  const leadsByStage = useMemo(() => {
    return filteredLeads.reduce<Record<string, Lead[]>>((acc, lead) => {
      const stage = lead.estagio_lead || "oportunidade"
      if (!acc[stage]) {
        acc[stage] = []
      }
      acc[stage].push(lead)
  ...
  const stageTotals = useMemo(() => {
    return Object.entries(leadsByStage).reduce<Record<string, number>>((acc, [stage, stageLeads]) => {
      acc[stage] = stageLeads.reduce((total, lead) => total + (lead.valor || 0), 0)
      return acc
    }, {})
  }, [leadsByStage])

  const getStageLimit = (stage: string) => stageLimits[stage] ?? KANBAN_INITIAL_STAGE_LIMIT

  const handleLoadMoreStage = async (stage: string) => {
    const user = getCurrentUser()
    if (!user) return

    const currentStageLeads = leads.filter((lead) => lead.estagio_lead === stage)
    const nextFrom = currentStageLeads.length
    const nextTo = nextFrom + KANBAN_STAGE_INCREMENT - 1
    const sellerFilter = (user.cargo || "").toLowerCase() === "vendedor" ? await getVendedorNameForUser(user) : undefined
    const stagePage = await getLeadsStagePage(user.id_empresa, stage, nextFrom, nextTo, { vendedor: sellerFilter })

    setLeads((prev) => {
      const existingIds = new Set(prev.map((lead) => lead.id))
      const nextLeads = stagePage.leads.filter((lead) => !existingIds.has(lead.id))
      return [...prev, ...nextLeads]
    })
    setStageCounts((prev) => ({ ...prev, [stage]: stagePage.count }))
    setStageLimits((prev) => ({
      ...prev,
      [stage]: Math.max(prev[stage] ?? KANBAN_INITIAL_STAGE_LIMIT, nextTo + 1),
    }))
  }

  const origens = useMemo(() => [...new Set(leads.map((lead) => lead.origem).filter(Boolean))], [leads])

  const vendedoresOptions = useMemo(() => {
    const vendedoresFallback = [...new Set(leads.map((lead) => lead.vendedor).filter(Boolean))].map((nome) => ({
      id: `lead-${nome}`,
      nome: nome as string,
    }))

    return vendedores.length > 0 ? vendedores : vendedoresFallback
  }, [leads, vendedores])
  ...
  return (
    <div className="space-y-6">
      ...
      {resumoMessage && (
        <Alert className={`${resumoMessage.type === "success" ? "border-green-500 bg-black" : "border-red-500 bg-black"}`}>
      ...
      {viewMode === "list" ? (
        ...
        <>
          ...
          <Card className="border-[#22C55E] bg-black">
            ...
            <div className="overflow-x-auto">
              <div className="flex min-w-max gap-4 pb-4">
                {visibleColumns.map((stage) => {
                  ...
                  const visibleStageLeads = stageLeads.slice(0, stageLimit)
                  const stageTotalCount = stageCounts[stage] ?? stageLeads.length
                  const hiddenStageLeads = Math.max(stageTotalCount - visibleStageLeads.length, 0)
                    ...
                    {(provided, snapshot) => (
                      ...
                      >
                        <CardHeader className="pb-3">
                          <CardTitle className="text-sm font-medium flex items-center justify-between">
                            ...
                            <div className="flex flex-col items-end gap-1">
                              <Badge variant="secondary" className="text-xs">
                                {stageTotalCount > visibleStageLeads.length
                                  ? `${visibleStageLeads.length}/${stageTotalCount}`
                                  : stageTotalCount}
                              </Badge>
                          ...
                          {snapshot.isDraggingOver && (
                            <div className="text-xs text-[#22C55E] font-medium animate-pulse">
                        ...
                        >
                          {visibleStageLeads.map((lead, index) => (
                                  ...
                                  <CardContent className="p-3">
                                    <div className="space-y-2">
                                      <div className="flex items-center justify-between text-foreground">
                                        <h4 className="font-medium text-sm text-white truncate flex-1">{lead.nome}</h4>
                          ...
                          {hiddenStageLeads > 0 && (
                            ...
                            >
                              Carregar mais {hiddenStageLeads} lead{hiddenStageLeads === 1 ? "" : "s"}
                            </Button>
                          )}
